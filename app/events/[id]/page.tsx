...
import { imageToBase64 } from '@/utils/placeholder';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${event.name || 'Eventas'} | Eventas`,
...
const Event = async ({ params, searchParams }: Props) => {
  ...
  const eventBlurImg = await imageToBase64(event.image.url);

  return (
    <main className="overflow-x-hidden text-center flex flex-col items-center justify-center  ">
      {event && (
        <>
          <section className="flex flex-col justify-center mb-24 items-center lg:mb-24">
            <h1 className="flex flex-col text-shadow mb-10 text-red-400 uppercase font-popins font-black">
              <span className="text-3xl min-[400px]:text-3xl sm:text-4xl md:text-6xl lg:text-7xl xl:text-8xl tracking-wide hyphens-auto ">
                <AnimatedLetters text={event.name} />
              </span>
              <span className="text-xl min-[400px]:text-2xl sm:text-3xl">
                <AnimatedLetters text={dateTimeToDisplay(event.date)} />
              </span>
            </h1>
          ...
          {event.bio && event.bio != '' ? (
            <section className="my-10 lg:my-5">
              ...
              >
                <span className="text-red-400 text-center text-3xl sm:text-5xl">
                  <FaLocationDot />
                </span>
              ...
              <AnimatedLines
                containerClass="text-base lg:text-xl mb-5 max-w-[70%] center"
